...
export function CompanyEmailDraft({ companyId }: { companyId: string }) {
  ...
  return (
    <div className="p-6 max-w-3xl">
      <div className="mb-4">
        <label className="text-xs text-muted-foreground block mb-1.5">Use Template</label>
        <select
      ...
      <div className="mb-4">
        ...
        <textarea
          ...
          rows={10}
          className="w-full px-3 py-2 text-sm border border-input bg-background rounded-md focus:outline-none focus:ring-2 focus:ring-ring resize-none leading-relaxed"
      ...
      <div className="flex gap-2">
        <button
          ...
          className="flex items-center gap-1.5 px-4 py-2 text-sm font-medium bg-primary text-primary-foreground rounded-md press-effect hover:opacity-90 transition-quart"
        >
          <Send className="h-3.5 w-3.5" /> Send
        </button>
        <button
          onClick={() => handleSend(true)}
          className="flex items-center gap-1.5 px-4 py-2 text-sm font-medium border border-border text-foreground rounded-md press-effect hover:bg-accent transition-quart"
        >
