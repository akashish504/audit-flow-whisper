  ...
  if (companyEmails.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-2">
        ...
        <p className="text-sm text-muted-foreground">No email threads for this entity</p>
      </div>
    );
  }
    ...
    <div className="h-full flex">
      <div className="w-72 border-r border-border flex flex-col shrink-0">
        <div className="px-3 py-2.5 border-b border-border">
          <p className="text-xs text-muted-foreground">{companyEmails.length} thread{companyEmails.length !== 1 ? 's' : ''}</p>
        </div>
        <div className="flex-1 overflow-auto">
          {companyEmails.map(e => (
            <button
              ...
              className={`w-full text-left px-3 py-3 border-b border-border hover:bg-accent/50 transition-quart ${selectedEmail === e.id ? 'bg-accent/50' : ''}`}
            >
              <div className="flex items-center gap-2 mb-1">
                {e.status === 'draft' ? <Clock className="h-3 w-3 text-warning shrink-0" /> : <Send className="h-3 w-3 text-success shrink-0" />}
                <span className="text-[10px] font-semibold uppercase text-muted-foreground">{e.status}</span>
              </div>
              <div className="text-sm font-medium text-foreground truncate">{e.subject}</div>
              <div className="text-xs text-muted-foreground mt-0.5">{new Date(e.timestamp).toLocaleDateString()}</div>
            </button>
          ))}
        </div>
      </div>
